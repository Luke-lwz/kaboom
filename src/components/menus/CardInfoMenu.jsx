...
function CardInfoMenu({ card, color, onCancel }) {
    ...
    return (
        <>
            <div className='w-full max-w-[23rem] h-fit rounded-xl flex flex-col items-center overflow-hidden bg-base-100 transition-all'>
                <div className='h-24 flex justify-start items-center w-full relative'>
                    <div style={{ backgroundColor: color.primary }} className='w-[3.5rem] h-24 overflow-hidden flex flex-col-reverse items-center justify-start'>
                        {card?.src && card.src !== "" && <img src={`/cards${card.src}`} alt="" className="w-full " />}

                    </div>
                    <div style={{ backgroundColor: color.secondary, color: color.text }} className='grow h-full flex flex-col justify-center items-start p-3'>
                ...
                <div style={{ background: `inear-gradient(to bottom, rgba(255,255,255,0), rgba(255,255,255,0), ${color.primary}25)` }} className='h-full w-full text-base-content p-4 pb-2.5 text-lg flex flex-col overflow-y-scroll scrollbar-hide leading-5 '>
